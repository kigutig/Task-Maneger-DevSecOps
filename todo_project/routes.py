...
# ==========================================
# CONFIGURAÇÃO DO AUDIT LOG VIA SYSLOG
# ==========================================
import logging
from logging.handlers import SysLogHandler

syslog_logger = logging.getLogger('syslog_devsecops')
syslog_logger.setLevel(logging.INFO)
# Conecta ao socket nativo de log do Linux (/dev/log)
try:
    handler = SysLogHandler(address='/dev/log')
    formatter = logging.Formatter('%(asctime)s - DevSecOpsApp - %(levelname)s - %(message)s')
    handler.setFormatter(formatter)
    syslog_logger.addHandler(handler)
except Exception:
    # Fallback caso o socket apresente variações no ambiente containerizado
    pass
# ==========================================
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if user and bcrypt.check_password_hash(user.password, form.password.data):
            ...
            syslog_logger.info(f"AUTENTICACAO_SUCESSO: Usuario '{form.username.data}' efetuou login com sucesso.")
            flash('Login Successfull', 'success')
            return redirect(url_for('all_tasks'))
        else:
            # REGISTRO DE AUDITORIA: Falha (Alvo de detecção de Força Bruta no SIEM)
            syslog_logger.warning(f"AUTENTICACAO_FALHA: Tentativa incorreta de login para o usuario '{form.username.data}'.")
